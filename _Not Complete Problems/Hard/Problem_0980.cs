    ...
    internal class Problem_0980
    {
        public class Solution
        {
            ...
            private int[][] Fill(int[][] grid, int i, int j, long step = 0)
            {
                ...
                // Вверх
                if (i > 0 && grid[i - 1][j] != -1) grid = Fill(grid, i - 1, j, step);
                // Вниз
                if (i < grid.Length - 1 && grid[i + 1][j] != -1) grid =  Fill(grid, i + 1, j, step);
                // Влево
                if (j > 0 && grid[i][j - 1] != -1) grid =  Fill(grid, i, j - 1, step);
                // Вправо
                if (j < grid[0].Length - 1 && grid[i][j + 1] != -1) grid = Fill(grid, i, j + 1, step);
